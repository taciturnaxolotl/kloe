...
import {
  gateApi, getSession, parseCookies, clientMetadata, resetAuthCache,
  handleLogin, handleCallback, handleLogout,
} from "../src/auth";
...
afterEach(() => { setConfig(null); resetAuthCache(); });

test("session store: create, read, delete, expire, sweep", () => {
  const s = memStore();
  s.createSession("sid1", "https://idp/u/kieran", { name: "Kieran", picture: "p.png" }, Date.now() + 60_000);
...
test("getSession reads the cookie and honors expiry", () => {
  ...
  expect(getSession(new Request("https://k/", { headers: { cookie: "kloe_session=sid" } }), store)?.sub).toBe("sub");
...
async function runLogin(store: Store): Promise<{ state: string; oauthCookie: string }> {
...
test("full flow: login → callback mints a session and redirects to returnTo", async () => {
  const idp = fakeIdp({ me: "https://idp/u/kieran", profile: { name: "Kieran", photo: "https://idp/k.png" } });
  try {
    enableAuth({ issuer: idp.url.origin, baseUrl: "https://kloe.test" });
    const store = memStore();
    const { state, oauthCookie } = await runLogin(store);

    const cb = new Request(`https://kloe.test/auth/callback?code=abc&state=${state}&iss=${encodeURIComponent(idp.url.origin)}`, {
      headers: { cookie: oauthCookie },
    });
    const res = await handleCallback(cb, store);
    expect(res.status).toBe(302);
    expect(res.headers.get("location")).toBe("/c/42"); // returnTo preserved
    const sid = res.headers.getSetCookie().find((c) => c.startsWith("kloe_session="))!.split(";")[0]!.split("=")[1]!;
...
test("callback rejects a state mismatch and an unauthorized sub", async () => {
  ...
  try {
    ...
    const bad = await handleCallback(
      new Request("https://kloe.test/auth/callback?code=abc&state=WRONG", { headers: { cookie: oauthCookie } }),
      store,
    );
    expect(bad.status).toBe(400); // state mismatch

    enableAuth({ issuer: idp.url.origin, baseUrl: "https://kloe.test", allowedSubs: ["https://idp/u/someone-else"] });
    const { state, oauthCookie: c2 } = await runLogin(store);
    const denied = await handleCallback(
      new Request(`https://kloe.test/auth/callback?code=abc&state=${state}`, { headers: { cookie: c2 } }),
      store,
    );
...
test("logout drops the session and clears the cookie", () => {
  const store = memStore();
  store.createSession("sid", "sub", {}, Date.now() + 60_000);
  const res = handleLogout(new Request("https://kloe.test/auth/logout", { headers: { cookie: "kloe_session=sid" } }), store);
  expect(res.status).toBe(302);
  expect(store.getSession("sid")).toBeUndefined();
  expect(res.headers.getSetCookie().some((c) => /kloe_session=;|kloe_session=; /.test(c) || c.startsWith("kloe_session=;"))).toBe(true);
});
