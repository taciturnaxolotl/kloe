...
test("GET /models returns all models hidden by default (opt-in)", async () => {
  const { app } = freshApp();
  const body = await json(await app.handle(new Request(`${base}/models`)));
  const refs = body.models.map((m: any) => m.ref);
  expect(refs).toContain("acme/acme-1");
  expect(refs).toContain("echo");
  // Nothing curated yet → everything hidden.
  expect(body.models.every((m: any) => m.visible === false)).toBe(true);
});

test("GET /models/chat is empty until a model is made visible", async () => {
  const { app } = freshApp();
  const body = await json(await app.handle(new Request(`${base}/models/chat`)));
  expect(body.models).toEqual([]);
});

test("PATCH /models makes a model visible and renames it; chat reflects it", async () => {
  const { app } = freshApp();

  const patched = await json(
    await app.handle(
      new Request(`${base}/models`, {
        method: "PATCH",
  ...
  const chat = await json(await app.handle(new Request(`${base}/models/chat`)));
...
test("PATCH is partial: a second patch keeps prior fields", async () => {
  ...
  await patch({ ref: "acme/acme-1", visible: true, displayName: "Kept" });
  // Only change sortOrder; visibility + name must persist.
  const result = await json(await patch({ ref: "acme/acme-1", sortOrder: 5 }));
...
test("PATCH displayName:null clears the override", async () => {
  ...
  await patch({ ref: "acme/acme-1", visible: true, displayName: "Temp" });
  const cleared = await json(await patch({ ref: "acme/acme-1", displayName: null }));
...
test("chat models are ordered by sortOrder then name", async () => {
  ...
  await patch({ ref: "acme/acme-1", visible: true, sortOrder: 10 });
  await patch({ ref: "acme/acme-2", visible: true, sortOrder: 1 });
  const chat = await json(await app.handle(new Request(`${base}/models/chat`)));
...
test("PATCH rejects an unknown model ref with 422", async () => {
  ...
  const res = await app.handle(
    new Request(`${base}/models`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ ref: "acme/ghost", visible: true }),
    }),
  );
